...
StateTreesSampler::StateTreesSampler(int maxXY,
                                     int n,
                                     int maxCopyEvents)
  : _maxXY(maxXY)
  , _n(n)
...
StateTreesSampler::StateTreeVector StateTreesSampler::sample()
{
  ...
  for (int c = 0; c < _n; ++c)
  {
    int numberOfNonNormalCopyStates = dis_02(g_rng);
...
//      std::cout << xy.first << "," << xy.second << std::endl;
    ...
    StateGraph::StateEdgeSetSet setS = StateGraph::getStateTrees(L, _maxXY);
  ...
  for (int c = 0; c < _n; ++c)
  {
    IntVector pi(stateToTriple.size(), -2);
    pi[0] = -1;
    ...
    for (int i = 0; i < kk; ++i)
    {
      ...
      SS.setCnState(i, triple._x, triple._y, triple._z);
    }
